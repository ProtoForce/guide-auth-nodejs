...
export const defaultWSClientSettings: WSClientSettings = {
...
export class WSClient {
    ...
    constructor(settings?: WSClientSettings) {
        this._state = WSClientState.Disconnected;
        this._settings = {
            ...defaultWSClientSettings,
            ...settings,
        };
        this._url = '';
        this._protocols = [];
        ...
        if (!WebSocket) {
            throw new Error(`In order to use WebSockets client, WebSocket must be present in the global namespace`);
        }
    }
    ...
    private tryOpen() {
        if (this._settings.maxReconnectAttempts && this._attempts >= this._settings.maxReconnectAttempts) {
            ...
            if (this.onDisconnect) {
                this.onDisconnect(CLOSE_NORMAL, false);
            }
            return;
        }
        ...
        ws.onclose = (event: CloseEvent) => {
            ...
            if (this._forceClose) {
                this._state = WSClientState.Disconnected;
                if (this.onDisconnect) {
                    this.onDisconnect(event.code, timedOut);
                }
                return;
            }
