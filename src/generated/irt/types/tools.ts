 ...
 */
export function blobToString(bytes: Uint8Array): string {
    let base64 = '';
 ...
 */
export function blobFromString(value: string): Uint8Array {
 ...
 */
export function nanosFromString(nanosString: string): number {
    if (nanosString.length < 3) {
        while (nanosString.length < 3) nanosString += '0';
    } else
    if (nanosString.length < 6) {
        while (nanosString.length < 6) nanosString += '0';
    } else
    if (nanosString.length < 9) {
        while (nanosString.length < 9) nanosString += '0';
    }
 ...
 */
export function nanosToString(nanos: number): string {
